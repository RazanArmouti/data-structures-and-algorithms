...
// Solution code here...
  return arr.indexOf(arr.reduce((strPrev,strNext)=> strPrev.length>strNext.length?strPrev:strNext,0));
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(str=> str.split('(').join('').split(' ').join('').split(')').join('').split('-').join('') );
};
