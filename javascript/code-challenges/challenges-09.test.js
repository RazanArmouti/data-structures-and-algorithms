...
const maxInArray = (arr) => {
  ...
  const maxValue = arr.reduce((accumulator, currentValue) => {
...
Write a function named getCourseKeys that takes in the courseInfo object
and returns an array containing the keys for the courseInfo object.
...
Write a function named checkValues that takes in an object
and a value and returns true if the value is in the object.
...
const hasChildrenValues = (arr, character) => {
  ...
  Object.values(arr).forEach(element => {
    if (element.children) {
      result.push(Object.values(element.name).join(''));
    }
  });
  return result.includes(character);
  // for (let i = 0; i < arr.length; i++) {
  //   let obj=arr[i];
  //   let properties = Object.keys(obj);
  //   properties.forEach(property => {
  ...
  //         console.log(obj.children[x]);
  //         if(obj.children[x]===character){
  //           return true;}
