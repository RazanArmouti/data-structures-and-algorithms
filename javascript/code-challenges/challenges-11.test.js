...
const count = (target, input) => {
  ...
  return input.reduce((acc,row)=>{
    let countInRow =row.reduce((acc1,value)=>{
      return value === target ?acc1+1:acc1;
    },0);
      ...
      while (j===idx && j<row.length){
        if(typeof(row[idx])!=='number'){
          ...
          if(row[idx] % 5 !== 0 ){
            ...
            row.splice(idx,1,Math.pow(2,row[idx]));
            j=j+1;
          }
        }
      }
...
let findShortest = (data) => {
  ...
  return data.reduce((acc,obj)=>{
    return Number(acc.height) - Number(obj.height) < 0 ? acc:obj;
  },0).name;
};
